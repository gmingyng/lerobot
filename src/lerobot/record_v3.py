...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        if policy is not None and preprocessor is not None and postprocessor is not None:
            ...
            # MODIFICATION START: Instead of skipping, generate a null action.
            # logging.info("No policy or teleoperator provided. Generating null actions (all zeros).")
            # print(dataset)
            # # 从数据集中获取期望的动作名称，以构建结构正确的动作字典
            # if dataset is not None:
            # print("robot ft: ", robot.action_features)
            # action_names = robot.action_features["action"]["names"]
            null_action = {name: 0.0 for name in robot.action_features}
            
            # # 原始代码期望一个 (action, observation) 元组作为处理器的输入
            # # 我们将输出命名为 act_processed_teleop，以便后续的代码块能够正确工作
            act_processed_teleop = teleop_action_processor((null_action, obs))
            # else:
            #     continue
            # logging.info(
        ...
        if dataset is not None:
            ...
            if len(history) > ACTION_HORIZON:
                ...
                future_state_as_action = {
                    name: obs_processed[name] for name in dataset.features["action"]["names"]
                }
                ...
                action_values = future_state_as_action
