...
class Piper(Robot):
    ...
    def get_piper_ctrl(self) -> dict[str, Any]:
        """
        Get current control commands sent to the Piper arm
        """
        if not self.is_connected:
            raise RuntimeError("Piper robot is not connected")
        ...
        # Get arm joint control commands
        joint_ctrl = self._piper.GetArmJointCtrl()
        gripper_ctrl = self._piper.GetArmGripperCtrl()

        # Parse joint control commands for the arm
        self._parse_joint_messages(joint_ctrl, "action", ctrl)

        # Parse gripper control command for the arm
        self._parse_gripper_messages(gripper_ctrl, "action", ctrl)
